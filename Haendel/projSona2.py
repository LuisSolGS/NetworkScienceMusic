...
def analise(G,name):
    ...
    print("The average in-degree is " + str(sum([deg[1] for deg in G.in_degree()])/n));
    print("The average out-degree is " + str(sum([deg[1] for deg in G.out_degree()])/n));
...
def ajuFit(matriz,dictio):
    ...
    for i in range(len(matriz)):
        ...
        if(nota in dictio):
            ...
            matriz[i] = (matriz[i][0], round(matriz[i][1] * 0.1,3));
        else:
